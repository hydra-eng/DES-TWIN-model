...
from app.db.connection import get_db, check_db_connection, get_db_stats
...
async def compare_scenarios(
    ...
    logger.info(
        ...
        scenario_name=scenario_config.scenario.name if scenario_config.scenario else "unnamed",
    )
    ...
    return {
        ...
        "demand_curve": {
            "base_arrivals_per_hour": [
                5, 3, 2, 2, 3, 6,      # 0-5 (night/early morning)
                12, 25, 35, 30, 25, 20,  # 6-11 (morning rush)
                15, 18, 20, 22, 28, 40,  # 12-17 (afternoon/evening start)
                45, 38, 25, 15, 10, 7,   # 18-23 (evening rush/night)
            ],
            "multipliers": {},
        },
        "scenario": {
