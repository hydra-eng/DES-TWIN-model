...
def load_real_network() -> Tuple[List[Dict[str, Any]], float]:
    ...
    if os.path.exists(inventory_path):
        try:
            ...
            if (
                "occupant" in df_inventory.columns
                and "batteryid" in df_inventory.columns
            ):
                inventory_counts = df_inventory.groupby("occupant")[
                    "batteryid"
                ].nunique()
    ...
    if os.path.exists(physics_path):
        try:
            ...
            if "discharging_time" in df_physics.columns:
                ...
                df_physics["discharging_time"] = pd.to_numeric(
                    df_physics["discharging_time"], errors="coerce"
                )
    ...
    print(
        f"[INFO] Loaded {len(stations)} Core Stations from Excel. Avg arrival: {avg_arrival_time_min:.2f} mins"
    )
    logger.info(
        "data_loader_success",
        station_count=len(stations),
        avg_arrival_min=avg_arrival_time_min,
    )
