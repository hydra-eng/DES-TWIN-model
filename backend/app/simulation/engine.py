...
from app.schemas.sim_config import (BaselineComparison, DemandCurve, EventType,
                                    Intervention, InterventionType,
                                    SimulationConfig, SimulationResult,
                                    SimulationStatus, StationConfig,
                                    StationKPI)
...
class SimulationOrchestrator:
    ...
    def __init__(self, config: SimulationConfig) -> None:
        ...
        if real_stations_data:
            ...
            self.config.stations = (
                real_stations  # Override provided stations with Real Data
            )
    ...
    def _apply_single_intervention(
        ...
        match intervention.type:
            ...
            case InterventionType.MODIFY_CHARGERS:
                target_id = intervention.target_station_id
                if target_id and target_id in stations:
                    stations[target_id].charger_count = intervention.parameters[
                        "new_count"
                    ]
                    logger.info(
    ...
    def _vehicle_arrival_generator(
        ...
        scenario_adjustments = (
            self.config.scenario.demand_adjustments if self.config.scenario else {}
        )
        ...
        while True:
            ...
            if (
                hasattr(self, "mean_arrival_time_min")
                and self.mean_arrival_time_min > 0
            ):
    ...
    def _aggregate_results(
        ...
        for station_id, station in self.stations.items():
            ...
            max_charger_time = (
                self.config.duration_days * 24 * 3600 * station.config.charger_count
            )
            utilization = (
                stats.charger_busy_time / max_charger_time
                if max_charger_time > 0
                else 0.0
            )
            ...
            idle_pct = (
                (avg_available / station.config.total_batteries) * 100
                if station.config.total_batteries > 0
                else 0.0
            )
        ...
        total_charger_capacity = (
            sum(s.config.charger_count for s in self.stations.values())
            * self.config.duration_days
            * 24
            * 3600
        )
        avg_utilization = (
            total_charger_time / total_charger_capacity
            if total_charger_capacity > 0
            else 0.0
        )

        # Calculate average idle inventory
        avg_idle = (
            sum(k.idle_inventory_pct for k in station_kpis) / len(station_kpis)
            if station_kpis
            else 0.0
        )
        ...
        logistics_cost = (
            len(self.stations) * self.config.duration_days * FIXED_COST_PER_STATION_DAY
        )

        opex_cost = energy_cost + depreciation_cost + logistics_cost

        scenario_name = (
            self.config.scenario.name if self.config.scenario else "baseline"
        )
