...
class Station:
    ...
    def __init__(
        ...
        self.chargers: simpy.Resource = simpy.Resource(env, capacity=config.charger_count)
    ...
    def _initialize_batteries(self) -> None:
        ...
        for i in range(self.config.total_batteries):
            ...
            if i < int(self.config.total_batteries * 0.8):
                ...
                status = BatteryStatus.CHARGING if initial_soc < 95 else BatteryStatus.AVAILABLE
    ...
    def handle_vehicle_arrival(
        ...
        available_count = sum(
            1 for b in self.battery_pool.items 
            if b.is_swappable
        )
        ...
        self._log_event(
            ...
            vehicle_id,
            {"battery_id": charged_battery.id, "duration": self.config.swap_time_seconds},
        )
