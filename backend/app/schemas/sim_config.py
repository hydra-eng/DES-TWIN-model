...
class BatteryConfig(BaseModel):
    ...
    capacity_kwh: float = Field(
        default=5.0, gt=0, description="Battery capacity in kWh"
    )
    min_swap_soc: float = Field(
        default=95.0, ge=80, le=100, description="Min SoC for swap eligibility"
    )
    max_soc: float = Field(
        default=100.0, ge=95, le=100, description="Max SoC when fully charged"
    )
    degradation_factor: float = Field(
        default=0.0001, ge=0, le=0.01, description="Health loss per cycle"
    )
...
class StationConfig(BaseModel):
    ...
    total_batteries: int = Field(
        default=20, ge=1, le=500, description="Total battery inventory"
    )
    charger_count: int = Field(
        default=4, ge=1, le=50, description="Number of charging bays"
    )
    charge_power_kw: float = Field(
        default=60.0, gt=0, le=500, description="Charger power in kW"
    )
    swap_time_seconds: int = Field(
        default=90, ge=30, le=600, description="Swap duration in seconds"
    )
    grid_power_limit_kw: float | None = Field(
        default=None, gt=0, description="Max grid power draw (None = unlimited)"
    )
    cooldown_seconds: int = Field(
        default=60, ge=0, le=300, description="Post-swap cooldown before charging"
    )
    ...
    status: str = Field(
        default="ACTIVE",
        description="Operational status: ACTIVE, INACTIVE, MAINTENANCE",
    )
...
class CalibrationParams(BaseModel):
    ...
    parking_delay_range: tuple[float, float] = Field(
        default=(5.0, 30.0), description="Random parking delay range in seconds"
    )
...
class DemandCurve(BaseModel):
    ...
    multipliers: dict[int, float] = Field(
        default_factory=dict, description="Hour -> multiplier overrides"
    )
...
class Intervention(BaseModel):
    ...
    target_station_id: str | None = Field(
        default=None, description="Target station for station-specific interventions"
    )
    parameters: dict[str, Any] = Field(
        default_factory=dict, description="Intervention parameters"
    )
...
class ScenarioConfig(BaseModel):
    ...
    base_scenario_id: UUID | None = Field(
        default=None, description="Parent scenario ID for baseline comparison"
    )
    interventions: list[Intervention] = Field(
        default_factory=list, description="List of interventions to apply"
    )
    demand_adjustments: dict[int, float] = Field(
        default_factory=dict, description="Hour -> demand multiplier adjustments"
    )
...
class SimulationConfig(BaseModel):
    ...
    duration_days: int = Field(
        default=1, ge=1, le=30, description="Simulation duration in days"
    )
    random_seed: int = Field(
        default=42, ge=0, description="Random seed for reproducibility"
    )
    demand_multiplier: float = Field(
        default=1.0, gt=0, le=10.0, description="Global demand scaling factor"
    )
    stations: list[StationConfig] = Field(
        ..., min_length=1, description="Station configurations"
    )
    demand_curve: DemandCurve = Field(
        default_factory=DemandCurve, description="Hourly demand profile"
    )
    calibration: CalibrationParams = Field(
        default_factory=CalibrationParams, description="Calibration parameters"
    )
    scenario: ScenarioConfig | None = Field(
        default=None, description="Optional scenario for what-if analysis"
    )
